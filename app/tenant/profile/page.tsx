...
import { User, Lock, Save, Building2, Link2 } from "lucide-react"
import { ChangePasswordModal } from "@/components/change-password-modal"
import { updateProfile, joinPgByCode } from "@/app/actions/profile"
import { PgConnectionForm } from "@/app/tenant/profile/pg-connection-form"
...
export default async function TenantProfilePage() {
  ...
  // 2. Fetch the tenant data matching the logged-in email (include pg_id)
  ...
  // 3. Fetch current PG details if tenant already linked
  let currentPg: { name: string; joining_code: string } | null = null
  if (tenant?.pg_id) {
    const { data: pg } = await supabase
      .from("pg_details")
      .select("name, joining_code")
      .eq("id", tenant.pg_id)
      .single()
    currentPg = pg
  }
        ...
        <Card className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/50 backdrop-blur-xl shadow-2xl">
          ...
          <CardContent className="p-6 sm:p-8">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-zinc-100">PG Connection</h3>
                <p className="text-sm text-zinc-400">
                  Enter the unique code provided by your PG owner to link your account.
                </p>
              </div>
            </div>

            <PgConnectionForm
              currentPgName={currentPg?.name ?? null}
              currentJoiningCode={currentPg?.joining_code ?? null}
            />
          </CardContent>
        </Card>
      </div>
