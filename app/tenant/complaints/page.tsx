...
function statusClasses(status: string) {
  switch (status.toLowerCase()) {
    ...
    case "pending":
      return "border-rose-500/20 bg-rose-500/10 text-rose-400 shadow-[0_0_10px_-3px_rgba(244,63,94,0.3)]"
    default:
...
export default async function TenantComplaintsPage() {
  ...
  // 2. Fetch complaints matching user's email
            ...
            <Table>
              <TableHeader className="bg-black/20">
                <TableRow className="border-white/5 hover:bg-transparent">
                  ...
                  <TableHead className="pr-6 h-11 text-[11px] font-semibold uppercase tracking-wider text-zinc-500 text-right">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {!complaints || complaints.length === 0 ? (
                  <TableRow className="border-white/5 hover:bg-transparent">
                    <TableCell colSpan={4} className="py-12 text-center text-zinc-500 italic">
                      No complaints found for {user.email}.
                    </TableCell>
                  </TableRow>
                ) : (
                  complaints.map((c) => (
